...
class WinAddStocks(QDialog, Ui_Dialog_Add_Stocks):
    ...
    def add_data(self):
        ...
        if status != "Загружен" and status != "Свободен":
            msg = QMessageBox()
            msg.setWindowTitle('Внимание')
            msg.setText('Статус введен неверно')
            msg.setIcon(QMessageBox.Warning)
            msg.exec_()
            return

        if not fullmatch(r"\d+", vmestimost):
            msg = QMessageBox()
            msg.setWindowTitle('Внимание')
            msg.setText('Вместимость должна быть указана числом')
            msg.setIcon(QMessageBox.Warning)
            msg.exec_()
            return
